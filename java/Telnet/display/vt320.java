 ...
 * --
 * $Id: vt320.java,v 1.61 1999/08/01 17:22:10 marcus Exp $
 *
 ...
 * </DL>
 * @version $Id: vt320.java,v 1.61 1999/08/01 17:22:10 marcus Exp $
...
public class vt320 extends Terminal implements TerminalHost
{
   ...
   */
  public String toString() { return "$Id: vt320.java,v 1.61 1999/08/01 17:22:10 marcus Exp $ "+display.version; }
  ...
  public String[][] getParameterInfo() {
    ...
    {"Remove", "String", 	"Programmable Remove key"},
    {"Insert", "String", 	"Programmable Insert key"},
    {"Home", "String", 		"Programmable Home key"},
    {"End", "String", 		"Programmable End key"},
    {"PgUp", "String", 		"Programmable Page Up key"},
    {"PgDown", "String", 	"Programmable Page Down key"},
    {"Up", "String", 		"Programmable Up Arrow key"},
    {"Down", "String", 		"Programmable Down Arrow key"},
    {"Left", "String", 		"Programmable Left Arrow key"},
    {"Right", "String",		"Programmable Right Arrow key"},
    };
    return pinfo;
  }
  ...
  public void addNotify() {
	...
	if (res!=null)
      ...
      for (int i=1;i<20;i++) {
        for (int j=0;j<4;j++) {
	  res= applet.getParameter(prefixes[j]+"F"+i);
	  if (res!=null) FunctionKey[i] = unEscape(res);
        }
      }
      for (int i=0;i<4;i++) {
	  res = applet.getParameter(prefixes[i]+"PGUP");
	  if (res!=null) PrevScn[i]=unEscape(res);
	  res = applet.getParameter(prefixes[i]+"PGDOWN");
	  if (res!=null) NextScn[i]=unEscape(res);
	  res = applet.getParameter(prefixes[i]+"END");
	  if (res!=null) KeyEnd[i]=unEscape(res);
	  res = applet.getParameter(prefixes[i]+"HOME");
	  if (res!=null) KeyHome[i]=unEscape(res);
	  res = applet.getParameter(prefixes[i]+"INSERT");
	  if (res!=null) Insert[i]=unEscape(res);
	  res = applet.getParameter(prefixes[i]+"REMOVE");
	  if (res!=null) Remove[i]=unEscape(res);
      }
      res = applet.getParameter("UP");if (res!=null) KeyUp=unEscape(res);
      res = applet.getParameter("DOWN");if (res!=null) KeyDown=unEscape(res);
      res = applet.getParameter("LEFT");if (res!=null) KeyLeft=unEscape(res);
      res = applet.getParameter("RIGHT");if (res!=null) KeyRight=unEscape(res);
  ...
  private void InitTerminalVars() {
    ...
    KeyHome[0] = KeyHome[1] = KeyHome[2] = KeyHome[3] = "\u0001bOH";
    KeyEnd[0]  = KeyEnd[1]  = KeyEnd[2]  = KeyEnd[3]  = "\u0001bOF";
 ...
 */
		...
		if (shift)
		    return host.send(Insert[0]); //  VMS shift delete = insert
		else
		    return host.send(Remove[0]); //  VMS delete = remove
	    }
            if (control) {
                switch(evt.key) {
                    case '0':
	...
	// the real key send.
	if (alt)
	    return host.send(""+((char)(evt.key|0x80)));
	if (capslock) {
	    // HACK: we should probably use some generic toupper() function
	    // if there is one
